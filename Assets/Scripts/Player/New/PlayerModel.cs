...
namespace Player.New
{
    ...
    public class PlayerModel : ScriptableObject
    {
        ...
        [Header("Movement")]
        [SerializeField, Tooltip("Velocidad de movimiento en suelo (m/s).")]
        ...
        [Header("Sprint")]
        [SerializeField, Tooltip("Multiplicador de velocidad al correr (moverse * X).")]
        ...
        public float SprintMinSpeedToKeep { get => sprintMinSpeedToKeep; set => sprintMinSpeedToKeep = value; }

        /// <summary>Duración de la ventana post-dash para armar el sprint.</summary>
        public float SprintArmWindow { get => sprintArmWindow; set => sprintArmWindow = value; }

        /// <summary>Tiempo de hold requerido para que el sprint comience.</summary>
        public float SprintHoldTime { get => sprintHoldTime; set => sprintHoldTime = value; }
        ...
        [Header("Jump")]
        [SerializeField, Tooltip("Impulso vertical del salto (m/s).")]
        ...
        [Header("Dash")]
        [SerializeField, Tooltip("Distancia total del dash (m).")]
        ...
        [SerializeField, Tooltip("Layers considerados enemigos para chequeos de hit.")]
        ...
        [SerializeField, Tooltip("Daño base por golpe.")] private float attackDamage = 10f;
        [SerializeField, Tooltip("Duración del ataque 1 (s).")] private float attack1Duration = 0.25f;
        [SerializeField, Tooltip("Duración del ataque 2 (s).")] private float attack2Duration = 0.28f;
        [SerializeField, Tooltip("Duración del ataque 3 (s).")] private float attack3Duration = 0.34f;
        ...
        [SerializeField, Tooltip("Radio del área de daño en el impacto (m).")] private float verticalAttackRadius = 2.8f;
        [SerializeField, Tooltip("Daño del ataque vertical.")] private float verticalDamage = 12f;
        [SerializeField, Tooltip("Empuje radial (m).")] private float verticalKnockbackDistance = 2.5f;
        [SerializeField, Tooltip("Stagger en enemigos (s).")] private float verticalStaggerTime = 0.35f;
        [SerializeField, Tooltip("Cooldown del ataque vertical (s).")] private float verticalAttackCooldown = 1.0f;
        [SerializeField, Tooltip("Tiempo inmóvil del jugador tras el impacto (s).")] private float verticalAttackPostStun = 0.25f;
        [SerializeField, Tooltip("Aceleración extra hacia abajo durante la caída (m/s²).")] private float verticalSlamExtraAccel = 40f;
        [SerializeField, Tooltip("Velocidad máxima hacia abajo (m/s).")] private float verticalSlamMaxDownSpeed = 30f;
        [SerializeField, Tooltip("Impulso vertical inicial negativo (m/s), 0 = sin impulso.")] private float verticalSlamStartDownSpeed = 0f;
        ...
        [SerializeField, Tooltip("Tiempo mínimo de carga (s).")] private float spinChargeMinTime = 0.6f;
        [SerializeField, Tooltip("Tiempo de carga para potencia/duración máximas (s).")] private float spinChargeMaxTime = 1.6f;
        [SerializeField, Tooltip("Mult. de movimiento mientras se ejecuta el SpinRelease.")] private float spinMoveSpeedMultiplierWhileExecuting = 0.6f;
        [SerializeField, Tooltip("Mult. de salto mientras dura el SpinRelease.")] private float spinJumpSpeedMultiplier = 1.15f;

        [Header("Spin: duraciones escalables")]
        [SerializeField, Tooltip("Duración mínima del giro (s).")] private float spinMinDuration = 0.55f;
        [SerializeField, Tooltip("Duración máxima del giro (s).")] private float spinMaxDuration = 1.10f;
        [SerializeField, Tooltip("Duración base (legacy).")] private float spinDuration = 0.7f;
        [SerializeField, Tooltip("Tiempo inmóvil luego del release (s).")] private float spinPostStun = 0.35f;

        [SerializeField, Tooltip("Radio del área de daño (m).")] private float spinRadius = 2.8f;
        [SerializeField, Tooltip("Daño del 360°.")] private float spinDamage = 10f;
        [SerializeField, Tooltip("Empuje radial (m).")] private float spinPushDistance = 2.0f;
        [SerializeField, Tooltip("Stagger aplicado (s).")] private float spinStaggerTime = 0.3f;
        [SerializeField, Tooltip("Cooldown (s).")] private float spinCooldown = 3.0f;
        [SerializeField, Tooltip("Multiplicador de movimiento mientras se carga.")] private float spinMoveSpeedMultiplierWhileCharging = 0.6f;

        [Header("Self Stun por Spin (escalable)")]
        [SerializeField, Tooltip("Luego del spin, el jugador queda knockdown.")] private bool spinCausesSelfStun = true;
        [SerializeField, Tooltip("Derribo mínimo (s).")] private float selfStunMinDuration = 0.60f;
        [SerializeField, Tooltip("Derribo máximo (s).")] private float selfStunMaxDuration = 2.00f;
        [SerializeField, Tooltip("Tiempo previo para 'GetUp' (s).")] private float selfStunGetUpLeadTime = 0.25f;
        ...
        public float SpinMoveSpeedMultiplierWhileExecuting { get => spinMoveSpeedMultiplierWhileExecuting; set => spinMoveSpeedMultiplierWhileExecuting = value; }
        public float SpinJumpSpeedMultiplier { get => spinJumpSpeedMultiplier; set => spinJumpSpeedMultiplier = value; }
        ...
        [Header("State Tuning")]
        [SerializeField, Tooltip("Coyote time al dejar el suelo (s).")]
        ...
        public float JumpGroundAirDetectDelay { get => jumpGroundAirDetectDelay; set => jumpGroundAirDetectDelay = value; }

        /// <summary>Retardo de detección de aire en <c>JumpAir</c> (s).</summary>
        public float JumpAirAirDetectDelay { get => jumpAirAirDetectDelay; set => jumpAirAirDetectDelay = value; }

        /// <summary>Tiempo de asentamiento del estado <c>Fall</c> (s).</summary>
        public float FallSettleTime { get => fallSettleTime; set => fallSettleTime = value; }
        ...
        [System.NonSerialized] private Vector2 _rawMoveInput;
        ...
        public Vector2 RawMoveInput { get => _rawMoveInput; set => _rawMoveInput = value; }

        /// <summary>Dirección de movimiento en espacio mundo (Vector3).</summary>
        public Vector3 MoveInputWorld { get => _moveInputWorld; set => _moveInputWorld = value; }
        ...
        public bool JumpWasPureVertical { get => _jumpWasPureVertical; set => _jumpWasPureVertical = value; }

        public bool  DashOnCooldown { get => _dashOnCooldown; set => _dashOnCooldown = value; }
        public float DashCooldownLeft { get => _dashCooldownLeft; set => _dashCooldownLeft = value; }

        public bool  SpinOnCooldown { get => _spinOnCooldown; set => _spinOnCooldown = value; }
        public float SpinCooldownLeft { get => _spinCooldownLeft; set => _spinCooldownLeft = value; }

        public bool  VerticalOnCooldown { get => _verticalOnCooldown; set => _verticalOnCooldown = value; }
        public float VerticalCooldownLeft { get => _verticalCooldownLeft; set => _verticalCooldownLeft = value; }

        public bool  AttackComboOnCooldown { get => _attackComboOnCooldown; set => _attackComboOnCooldown = value; }
        public float AttackComboCooldownLeft { get => _attackComboCooldownLeft; set => _attackComboCooldownLeft = value; }

        /// <summary>Ratio [0..1] de carga del spin (se setea en SpinCharge).</summary>
        public float SpinChargeRatio { get => _spinChargeRatio; set => _spinChargeRatio = value; }

        /// <summary>Si el jugador está en estado de SelfStun (knockdown).</summary>
        public bool  IsSelfStunned { get => _isSelfStunned; set => _isSelfStunned = value; }

        /// <summary>Tiempo restante del SelfStun actual (s).</summary>
        public float SelfStunTimeLeft { get => _selfStunTimeLeft; set => _selfStunTimeLeft = value; }

        /// <summary>Duración objetivo del SelfStun actual (s).</summary>
        public float SelfStunDuration { get => _selfStunDuration; set => _selfStunDuration = value; }

        /// <summary>Ventana post-dash armada para iniciar sprint.</summary>
        public bool  SprintArmed { get => _sprintArmed; set => _sprintArmed = value; }

        /// <summary>Estado de botón de dash mantenido (para hold de sprint).</summary>
        public bool  DashHeld { get => _dashHeld; set => _dashHeld = value; }

        /// <summary>Tiempo restante de la ventana de sprint (s).</summary>
        public float SprintArmTimeLeft { get => _sprintArmTimeLeft; set => _sprintArmTimeLeft = value; }

        /// <summary>Acumulador de hold (s) mientras se mantiene el botón.</summary>
        public float SprintHoldCounter { get => _sprintHoldCounter; set => _sprintHoldCounter = value; }
        ...
        {
            _dashOnCooldown = false; _dashCooldownLeft = 0f;
            _spinOnCooldown = false; _spinCooldownLeft = 0f;
            _verticalOnCooldown = false; _verticalCooldownLeft = 0f;
            _attackComboOnCooldown = false; _attackComboCooldownLeft = 0f;
        }
