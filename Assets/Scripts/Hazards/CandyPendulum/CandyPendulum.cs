...
namespace Hazards.CandyPendulum
{
    ...
    public class CandyPendulum : MonoBehaviour
    {
        ...
        [Header("Oscilación")] [SerializeField, Tooltip("Ángulo máximo (±) en grados")]
        ...
        [Header("Daño")] [SerializeField] private int damage = 1;
        [SerializeField] private Knockback knockback = new Knockback { horizontal = 20, vertical = 35 };

        [Header("Detección")] [SerializeField, Tooltip("Capas consideradas jugador")]
        private LayerMask playerMask = 0;

        [Header("Anti multi-hit")] [SerializeField, Tooltip("Tiempo mínimo entre golpes a la misma víctima")]
        private float rehitCooldown = 0.25f;

        private bool _emittedAudio = false;
        private float _time;
        ...
        private void Update()
        {
            ...
            if (swingAngle - MathF.Abs(angle) < 5)
            {
                if (!_emittedAudio)
                {
                    onStartSwing?.Invoke();
